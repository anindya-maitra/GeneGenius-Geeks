...
for i in range(len(drugv)):
    ...
    output_drugfour.loc[i]=" ".join(map(str,drugv[i]))
    thlen=0
    while (thlen+thsize)<faultsize:
        ...
        output_drugfour.iloc[i,thlen+1:thlen+thsize+1]=env_gpu.get()
        thlen=thlen+thsize
    ...
    if left>0:
        ...
        output_drugfour.iloc[i,thlen+1:]=env_gpu.get()

output_drugfour.to_csv("outputDrugFourP.csv")
