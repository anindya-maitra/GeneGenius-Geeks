...
df_drugthree=pd.read_csv("outputDrugThreeP.csv",index_col=0)

col_df=len(df_drugthree.columns[1:])
len_df=len(df_drugthree.index)

#calculate Probabilistic Score
df_con=cond.condense(df_drugthree)
df_con.to_csv("output_3_pscore.csv")
df_con=pd.read_csv("output_3_pscore.csv",index_col=0)
